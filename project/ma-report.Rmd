...
| Data                    | Accuracy |
|-------------------------|----------|
| Logistical Regression   | 0.7826   | 

### 3. Decision Tree
The Decision Tree algorithm is another supervised model that can be used for classification tasks. The algorithm learns classes by hierarchically slicing the features, so that it improves overall target metric. The strengths of this algorithm relies on the simplicity of it. They are easy to interpret, can deal categorical variables efficiently, and can deal noninformative features. The downsides of this algorithm is, being prone to overfit if not dealt properly, and sensivity to the training data and its variance. Since majority of our features are categorical, we decided that applying decision trees could be beneficial.

As it mentioned above, if we let decision tree to grow its full potential without any constraint, it would create a leaf node for each unique data instance, resulting in the complete overfitting to the train data. Therefore, there are two methods to prevent that. The first method, is to apply restrictive constraints to the tree while it is growing such as limitting the minimum number of data instances for each leaf nodes and splittable nodes, or limitting the maximum tree depth. Although, this method prevents overfitting, it can lead to underfitting by not growing the tree even if it is highly beneficial split due to the growing limits. The second method is more robust in those cases, where the tree is grown to its full capacity, and pruned afterwards. 
